...
function verDetalle(r) {
  ...
  (window.SECTIONS || []).forEach(sec => {
    const sid  = sec.id;
    const cols = sec.col_keys || [sid+'_sunec', sid+'_coripuno', sid+'_antioquia', sid+'_parcoy'];
    const plts = sec.plants   || ['SUNEC', 'CORI PUNO', 'ANTIOQUIA', 'PARCOY'];

    const vals4 = cols.map(c => parseFloat(r[c] || 0));
    ...
    tr.innerHTML = `
      ...
      ${vals4.map((v, i) => `<td style="text-align:right"><span style="display:block;font-size:.7rem;color:var(--text-muted);margin-bottom:2px">${plts[i]}</span>${fmt(v)}</td>`).join('')}
