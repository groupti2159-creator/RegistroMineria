...
def dashboard():
    ...
    registros = []
    for row in rows:
        r = {k.lower(): _serialize(v) for k, v in row.items()} if isinstance(row, dict) \
            else {keys[i]: _serialize(row[i]) for i in range(len(keys))}
        r['total_cp'] = sum(float(r.get(s["col"], 0) or 0) for s in SECTIONS_CP if s["unit"] == "Kg")
        registros.append(r)

    return render_template('gestion_residuos/dashboard.html',
