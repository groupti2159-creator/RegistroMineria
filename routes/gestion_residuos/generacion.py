...
SECTIONS = [
    ...
    {"id": "pap",  "label": "RRSS Papel y cartón",   "unit": "Kg",  "en_total": True,
     "col_keys": ["pap_coripuno"], "plants": ["CORI PUNO"]},
...
PLANT_COLUMN_MAP = {
    'SUNEC': 'sunec', 'CORI PUNO': 'coripuno',
    'ANTIOQUIA': 'antioquia', 'PARCOY': 'parcoy', 'SANTA MARÍA': 'smaria',
}
...
def sec_cols(sec):
    if 'col_keys' in sec:
        return sec['col_keys']
    sid = sec['id']
    return [f"{sid}_{PLANT_COLUMN_MAP.get(p, p.lower().replace(' ', '_'))}" for p in PLANTS]
...
def serialize_registro(r):
    ...
    for k, v in r.items():
        key = k.lower()
        if v is None: out[key] = None
        elif isinstance(v, Decimal): out[key] = float(v)
        elif isinstance(v, (datetime, date)): out[key] = v.isoformat()
        else: out[key] = v
    return out
...
def generacion():
    ...
    for r in registros:
        for sec in SECTIONS:
            ...
            if not r.get(total_key):
                r[total_key] = round(sum(float(r.get(c) or 0) for c in col_keys), 2)
    return render_template('gestion_residuos/generacion.html',
        sections=SECTIONS, plants=PLANTS, colors=COLORS,
        today=date.today().isoformat(),
        notif_count=get_notif_count(), registros=registros)


@gr_bp.route('/rrss/calcular', methods=['POST'])
@admin_required
def calcular():
    ...
    for sec in SECTIONS:
        ...
        secciones[sid] = {'total': total, 'unit': sec['unit']}
        if sec['en_total']:
            total_kg += total
    return jsonify({'secciones': secciones, 'total_kg': total_kg,
                    'generacion_t': f'{total_kg / 1000:.3f}'})


@gr_bp.route('/rrss/guardar', methods=['POST'])
@admin_required
def guardar():
    data    = request.get_json() or {}
...
@gr_bp.route('/rrss/eliminar/<int:idgeneracion>', methods=['POST'])
@admin_required
...
@gr_bp.route('/rrss/editar/<int:idgeneracion>', methods=['POST'])
@admin_required
