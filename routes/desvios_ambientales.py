...
def dashboard():
    ...
    registros_pendientes = [r for r in todos if r.get(
    ...
    return render_template('desvios_ambientales/dashboard.html',
                           stats=stats or {
                               'total': 0, 'culminados': 0, 'en_proceso': 0, 'pendientes': 0},
...
def registrar():
    ...
    estados_unicos = sorted(
        list(set([r.get('estado', '') for r in todos_registros if r.get('estado')])))

    try:
        cur = mysql.connection.cursor()
        registros = sp_exec(cur, 'sp_listarregistros',
                            (estado_filter or None,))
    finally:
        if 'cur' in locals() and cur:
            cur.close()

    if personal_filter:
        registros = [r for r in registros if r.get(
            'personalresponsable', '').lower() == personal_filter.lower()]
    ...
    registros_ordenados = sorted(
        registros, key=lambda x: orden_estados.get(x.get('estado', ''), 999))
...
def estadisticas_tipos():
    ...
    try:
        cur = mysql.connection.cursor()
        stats = sp_exec(cur, 'sp_estadisticas_tipos_pendientes',
                        (fecha_ini, fecha_fin))
    finally:
        if 'cur' in locals() and cur:
            cur.close()
    ...
    try:
        ...
        try:
            ...
            result = sp_one(cur, 'sp_crearregistro', (
                codigo,
                request.form.get(
                    'fecha_inicio') or datetime.now().strftime('%Y-%m-%d'),
                request.form.get('fecha_ejecucion') or None,
                request.form['descripcion'], request.form.get('accion', ''),
                int(request.form['area_reportante']), int(
                    request.form['area_responsable']),
                int(request.form['ubicacion']), int(
                    request.form['riesgo']), int(request.form['tipo']),
        ...
        for f in request.files.getlist('evidencias')[:5]:
            if f and f.filename:
                ruta, nombre, kb = save_image(
                    f, 'static/uploads', 'evidencias')
                if ruta:
                    try:
                        cur = mysql.connection.cursor()
                        sp_exec(cur, 'sp_guardarimagen', (rid,
                                session['usuario_rol'], 1, 1, ruta, nombre, kb))
                        mysql.connection.commit()
                    finally:
                        if 'cur' in locals() and cur:
                            cur.close()
        ...
        for f in request.files.getlist('levantamientos')[:5]:
            if f and f.filename:
                ruta, nombre, kb = save_image(
                    f, 'static/uploads', 'levantamientos')
                if ruta:
                    try:
                        cur = mysql.connection.cursor()
                        sp_exec(cur, 'sp_guardarimagen', (rid,
                                session['usuario_rol'], 2, 1, ruta, nombre, kb))
                        mysql.connection.commit()
                    finally:
                        if 'cur' in locals() and cur:
                            cur.close()
        ...
        try:
            cur = mysql.connection.cursor()
            cur.execute(
                "SELECT ur.idusuariorol FROM tbl_usuariorol ur JOIN tbl_roles r ON r.idroles=ur.idroles WHERE r.nombrerol IN ('Supervisor','Trabajador')")
            sups = cur.fetchall()
        finally:
            if 'cur' in locals() and cur:
                cur.close()
        for s in sups:
            try:
                cur = mysql.connection.cursor()
                sp_exec(cur, 'sp_crearnotificacion',
                        (s['idusuariorol'], f'Nuevo reporte {codigo} creado', 'info', rid))
                mysql.connection.commit()
            finally:
                if 'cur' in locals() and cur:
                    cur.close()
    ...
    try:
        ...
        try:
            ...
            result = sp_exec(cur, 'sp_actualizarregistro', (
                ...
                request.form.get('personal_responsable', ''),
                int(request.form['ccta_responsable']) if request.form.get(
                    'ccta_responsable', '').strip() not in ('', '0', 'None') else 0,
                request.form.get('dni_responsable', '').strip()
            ))
            mysql.connection.commit()
        finally:
        ...
        if imagenes_eliminar:
            for imagen_id in [i.strip() for i in imagenes_eliminar.split(',') if i.strip()]:
                try:
                    cur = mysql.connection.cursor()
                    cur.execute(
                        "SELECT rutaimagen FROM tbl_imagenregistro WHERE idimagen=%s", (imagen_id,))
...
def eliminar_registro(rid):
    try:
        try:
            cur = mysql.connection.cursor()
            cur.execute(
                "SELECT rutaimagen FROM tbl_imagenregistro WHERE idregistro=%s", (rid,))
            imagenes = cur.fetchall()
            cur.execute(
                "DELETE FROM tbl_historialaprobacion WHERE idregistro=%s", (rid,))
            cur.execute(
                "DELETE FROM tbl_notificacion WHERE idregistro=%s", (rid,))
        ...
        for img in imagenes:
            ...
            if ruta:
                filepath = os.path.normpath(
                    os.path.join(base_dir, 'static', ruta))
                try:
...
def detalle_registro(rid):
    ...
    def serialize(obj):
        ...
        for k, v in obj.items():
            key = k.lower()
            out[key] = v.strftime('%Y-%m-%d') if hasattr(v,
                                                         'strftime') else (v if v is not None else '')
        return out

    imgs_serial = [serialize(i)
                   for i in imagenes if i.get('idEstadoImagen') != 3]
    ...
    try:
        ...
        if decision == 'APROBADA':
            ...
            if not imagenes_ids:
                ...
                return redirect(url_for('da.registrar'))
            ids_list = [i.strip()
                        for i in imagenes_ids.split(',') if i.strip()]
            imagenes_ids_rechazar = request.form.get(
                'imagenes_ids_rechazar', '')
            if imagenes_ids_rechazar:
                ids_rechazar_list = [
                    i.strip() for i in imagenes_ids_rechazar.split(',') if i.strip()]
            ...
            for imagen_id in ids_list:
                try:
                    cur = mysql.connection.cursor()
                    sp_exec(cur, 'sp_validarimagen', (imagen_id, rid,
                            session['usuario_rol'], 'APROBADA', comentario))
                    mysql.connection.commit()
                finally:
                    if 'cur' in locals() and cur:
                        cur.close()
            ...
            if not imagenes_ids_rechazar:
                ...
                return redirect(url_for('da.registrar'))
            ids_list = [i.strip()
                        for i in imagenes_ids_rechazar.split(',') if i.strip()]
            # Obtener rutas antes de marcar como rechazadas
            rutas_rechazadas = {}
            if ids_list:
                try:
                    cur = mysql.connection.cursor()
                    cur.execute("SELECT idimagen, rutaimagen FROM tbl_imagenregistro WHERE idimagen IN (%s)" % ','.join(
                        ['%s']*len(ids_list)), ids_list)
                    for row in cur.fetchall():
                        rutas_rechazadas[str(row['idimagen'])] = row.get(
                            'rutaimagen', '')
                finally:
                    if 'cur' in locals() and cur:
                        cur.close()
            for imagen_id in ids_list:
                try:
                    cur = mysql.connection.cursor()
                    sp_exec(cur, 'sp_validarimagen', (imagen_id, rid,
                            session['usuario_rol'], 'RECHAZADA', comentario))
                    mysql.connection.commit()
                finally:
                    if 'cur' in locals() and cur:
                        cur.close()
            ...
            flash(
                f'{len(ids_list)} imagen(es) rechazada(s). El supervisor debe volver a subir imagenes', 'warning')
        ...
        if imagenes_ids_rechazar:
            ...
            try:
                cur = mysql.connection.cursor()
                cur.execute("SELECT DISTINCT idusuariorol FROM tbl_imagenregistro WHERE idimagen IN (%s)" % ','.join(
                    ['%s']*len(ids_notif)), ids_notif)
            ...
            for usuario in usuarios:
                ...
                try:
                    cur = mysql.connection.cursor()
                    sp_exec(cur, 'sp_crearnotificacion', (
                        usuario['idusuariorol'], msg, 'success' if decision == 'APROBADA' else 'warning', rid))
                    mysql.connection.commit()
                finally:
                    if 'cur' in locals() and cur:
                        cur.close()
...
def exportar_excel():
    try:
        ...
        for row in rows:
            try:
                cur = mysql.connection.cursor()
                imagenes = sp_exec(cur, 'sp_imagenesregistro',
                                   (row.get('IdRegistro', ''),))
            finally:
                if 'cur' in locals() and cur:
                    cur.close()
            ...
            vals = [row.get('Codigo', ''), row.get('Fecha', ''), row.get('FechaEjecucion', ''),
                    row.get('AreaReportante', ''), row.get(
                        'Ubicacion', ''), row.get('Descripcion', ''),
                    row.get('DescripcionTipo', ''), row.get(
                        'Riesgo', ''), row.get('Estado', ''),
                ...
                for col_idx, img_list in [(14, evidencias), (15, levantamientos)]:
                    if img_idx < len(img_list):
                        ...
                        possible = [img_path,
                                    ...
                                    os.path.join(
                                        'static', img_path.replace('static/', '')),
                                    os.path.join('ecosupervisor', 'static', img_path.replace('static/', ''))]
                        full_path = next(
                            (p for p in possible if os.path.exists(p)), None)
                        if full_path:
                            try:
                                pil_img = PILImage.open(full_path)
                                pil_img.thumbnail(
                                    (80, 80), PILImage.Resampling.LANCZOS)
                                ...
                                ws.add_image(
                                    xl_img, f'{get_column_letter(col_idx)}{img_row}')
                            except Exception as e:
                                ws.cell(row=img_row, column=col_idx,
